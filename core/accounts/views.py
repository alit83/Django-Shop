...
class ResendVerificationEmailView(LoginRequiredMixin,View):
    def post(self,request,*args,**kwargs):
        ...
        if user.is_verified:
            ...
            return redirect('/')
        now = timezone.now()
        if user.last_verification_email and now - user.last_verification_email < timedelta(minutes=2):
            remaining = 120 - int((now - user.last_verification_email).total_seconds())
...
class ForgetPasswordView(OnlyUnAuthenticatedPermission ,FormView ):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.success(self.request , "ایمیل تغییر رمز عبور با موفقیت برایتان ارسال شد.")
