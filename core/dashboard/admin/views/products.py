...
from shop.models import Product , ProductCategory , ProductImage , AttributeGroup , Attribute , ProductAttribute
...
class AdminProductEditView(HasAdminAccessPermission,SuccessMessageMixin,LoginRequiredMixin,UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy("dashboard:admin:products-edit" , kwargs={"pk":self.get_object().pk})
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["image_form"] = AdminCreateImageForm()
        product=self.get_object()
        attribute_group= AttributeGroup.objects.filter(attributes_group__attribute_attributes__product=product,category=product.category).distinct()
        attributes_q =attribute_group.prefetch_related(Prefetch(
        'attributes_group',
        queryset=Attribute.objects.filter(attribute_attributes__product=product),
        to_attr='product_attributes'))
        context["attributes_group"]= attributes_q.prefetch_related(Prefetch(
...
class AdminProductDeleteImageView(LoginRequiredMixin, HasAdminAccessPermission, SuccessMessageMixin, DeleteView):
    ...
    def get_success_url(self):
        return reverse_lazy("dashboard:admin:products-edit" , kwargs={"pk":self.kwargs.get('product_pk')})
...
class AdminProductCreateView(LoginRequiredMixin, HasAdminAccessPermission, SuccessMessageMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        return redirect(reverse_lazy("dashboard:admin:products-edit", kwargs={"pk": form.instance.pk}))
...
class AdminProductCreateImageView(LoginRequiredMixin, HasAdminAccessPermission, SuccessMessageMixin, CreateView):
    http_method_names = ["post"]
    queryset = ProductImage.objects.all()
    ...
    def form_valid(self, form):
        ...
        return redirect(reverse_lazy("dashboard:admin:products-edit", kwargs={"pk": self.kwargs.get('pk')}))
