...
def cartitem(db, cart, product):
    return CartItemModel.objects.create(
        cart=cart, product=product, quantity=2.0
    )
...
@pytest.mark.django_db
def test__post__order_checkout_view_successful_response(
    mock_zarinpal_success, client, user, address, product, cart, cartitem
):
    client.force_login(user)
    url = reverse("order:checkout")
    data = {"address_id": address.id}
    client.post(url, data)
    assert OrderModel.objects.filter(user=user).exists() is True
    order = OrderModel.objects.get(user=user)
    assert order.status == OrderStatusType.pending.value
    assert OrderItemModel.objects.filter(order=order).exists() is True
