...
from ..models import Product, ProductCategory ,ProductStatus , WishlistProductModel
...
def test_AddOrRemoveWishlist_view_successful_response(client,product , user):
    ...
    assert response.status_code == 200
    assert WishlistProductModel.objects.filter(product_id=product.id).exists() == True