...
from shop.models import Product , ProductStatus , ProductCategory , WishlistProductModel , ProductAttribute , Attribute
...
class ShopProductsGridView(ListView):
    ...
    def get_queryset(self):
        ...
        queryset = Product.objects.filter(category__slug=self.cat.slug , status=ProductStatus.publish.value)
        ...
        if attributes_filter:
            ...
            for attr_id , attr_value in attributes_filter.items():
                subquery_attrs = ProductAttribute.objects.filter(attribute__id=int(attr_id),value=attr_value,product_id=OuterRef('pk')).values("product_id")
    ...
    def get_context_data(self, **kwargs):
        ...
        attributes = Attribute.objects.filter(group__category=self.cat).select_related("group__category")
        for attr in attributes:
            attr.distinct_values = attr.attribute_attributes.values_list('value', flat=True).distinct()
...
class ShopProductsCategoryView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context= super().get_context_data(**kwargs)
        context["parents_categories"] = ProductCategory.objects.filter(parent=None)
        categories=ProductCategory.objects.filter(category_child= None).order_by('-created_date')[:2]
...
class ShopProductMainView(ListView):
    ...
    def get_queryset(self):
        ...
        queryset = Product.objects.filter(category__parent__parent__slug=cat.slug , status=ProductStatus.publish.value).select_related("category__parent__parent")
        return queryset
    def get_context_data(self, **kwargs):
        context=super().get_context_data(**kwargs)
        context["category_slug"]=self.slug
        categories=ProductCategory.objects.filter(parent__parent__slug=self.slug).select_related("parent__parent").order_by('-created_date')[:3]
        for category in categories:
            category.new_products = category.category_product.filter(status=ProductStatus.publish.value)[:3]
            category.min_price = category.category_product.filter(status=ProductStatus.publish.value).order_by("price")[:1]
        context["categories"]=categories
...
class ShopProductMiddleView(ListView):
    ...
    def get_queryset(self):
        slug = self.kwargs.get('slug')
        self.cat = get_object_or_404(ProductCategory,slug=slug)
        queryset = Product.objects.filter(category__parent__slug=self.cat.slug , status=ProductStatus.publish.value).select_related("category__parent")
        query_params = self.request.GET
        if search_q := query_params.get("q"):
            queryset = queryset.filter(title__icontains=search_q)
        if min_price:=query_params.get("min_price"):
        ...
        if attributes_filter:
            combined_attribute_q = Q()
            for attr_id , attr_value in attributes_filter.items():
                subquery_attrs = ProductAttribute.objects.filter(value=attr_value,product_id=OuterRef('pk')).values("product_id")
                combined_attribute_q &= Q(id__in=Subquery(subquery_attrs))
            queryset=queryset.filter(combined_attribute_q)
        return queryset
    def get_context_data(self, **kwargs):
        ...
        attributes_per_cat = [
        set(
            Attribute.objects.filter(group__category=cat).select_related("group__category")
            .values_list("title", flat=True)
        )
        for cat in cat_child
    ]
        common_titles = set.intersection(*attributes_per_cat) if attributes_per_cat else set()
        if not common_titles:
            context["attributes"] = []
            return context
        distinct_values_subquery = (
        ProductAttribute.objects
        .filter(attribute__title=OuterRef("title"),attribute__group__category__in=cat_child).select_related("attribute__group__category")
        .order_by()
        ...
        .annotate(distinct_values=Subquery(distinct_values_subquery))).distinct('title')
        context["attributes"]=attributes
        return context
